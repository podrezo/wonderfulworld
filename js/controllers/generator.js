(function() {
  ...
  app.controller('GeneratorController', ['$scope', 'utilityService', 'PlacesDatabase',
    function($scope, util, PlacesDatabase) {
      $scope.countries = [
        {
          "name": "Afghanistan",
          "alpha-2": "AF",
      ...
      PlacesDatabase.data().then(function(db) {
        $scope.tagSuggestions = db[1].tags;
      });
      $scope.toggleTag = function(tag) {
        if(_.contains($scope.place.properties.tags,tag)) {
          $scope.place.properties.tags = _.without($scope.place.properties.tags, tag);
        } else {
          $scope.place.properties.tags.push(tag);
        }
        $scope.updateModel();
      };
      $scope.addTag = function() {
        ...
        if(_.every($scope.tagSuggestions, function(existingTag) { return existingTag.tag.toLowerCase() !== $scope.addTagText.toLowerCase(); })) {
          $scope.tagSuggestions.push({
            tag: $scope.addTagText,
            quantity: 0,
            alreadyChecked: true
          });
          $scope.toggleTag($scope.addTagText, true);
        } else {
          alert('Tag is already in the checklist above.');
        }
        $scope.addTagText = '';
      };
      $scope.$watch('place.geometry.coordinates[0]', function() {
        $scope.friendlyLon = util.getDD2DMS($scope.place.geometry.coordinates[0],'lon');
        $scope.updateModel();
        $scope.remapCoords();
      });
      $scope.$watch('place.geometry.coordinates[1]', function() {
        $scope.friendlyLat = util.getDD2DMS($scope.place.geometry.coordinates[1],'lat');
        $scope.updateModel();
        $scope.remapCoords();
      });
      $scope.$watch('place.geometry.coordinates[2]', function() {
        $scope.updateModel();
      });
      $scope.$watchCollection('place.properties', function() { $scope.updateModel() });
      $scope.remapCoords = function() {
        if(mapMarkerObject) map.remove(mapMarkerObject);
        mapMarkerObject = po.geoJson().features([$scope.place]).on("load", util.convertMarkers(po));
        map.add(mapMarkerObject);
        map.center({
      ...
      $scope.shadowCopy = function() {
        ...
        // strip elevation if it is zero
        if(pl.geometry.coordinates[2] === 0) {
          pl.geometry.coordinates = pl.geometry.coordinates.splice(0,2);
        }
        // strip contributedBy if it is empty
        if(!pl.properties.contributedBy) {
      ...
      $scope.updateModel = function() {
        $scope.placeGeoJson = JSON.stringify($scope.shadowCopy(), null, 2);
